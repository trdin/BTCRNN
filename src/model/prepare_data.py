...
def prepare_data(
    path_to_data,
    pipeline=Pipeline(
        steps=[("scaler", MinMaxScaler()), ("model", RandomForestRegressor())]
    ),
):
    # Read the data
    ...
    columns_complete_values = all_data.drop(
        columns_with_missing_values + ["date"], axis=1
    ).columns.tolist()
    ...
    learn_features = all_data[
        ["BTC Price"] + selected_features
    ]  # 'BTC Price' is chosen as the target variable for this example

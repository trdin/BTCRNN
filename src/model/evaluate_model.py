...
def evaluate_model(data_path, mode,windowsize = 24):

   

    dagshub.auth.add_app_token(token=env.mlflow_tracking_password)
    dagshub.init("RNNSistem", env.mlflow_tracking_username, mlflow=True)
    ...
    data_normalized = np.column_stack([stands_normalized, np.array(learn_features[:, 1:])])
    prod_data_normalized = np.column_stack([prod_stands_normalized, np.array(learn_features[:, 1:])])
    ...
    tm.save_test_metrics(lstm_mae_adv, lstm_mse_adv, lstm_evs_adv, './reports/'+mode+'/metrics.txt')
